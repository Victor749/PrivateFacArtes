...
router.get('/api/json/:id_obra', function (req, res) {
  var sql = "select * from obra where idObra=".concat(req.params.id_obra);
...
router.get('/api/comentarios/:idObra/:actual/:limit', function (req, res) {
  ...
  var sql = "select  idComentario, idUsuario, nombreUsuario, contenido, linkFoto, plataforma, fecha from obra,comentario where obra.idObra=".concat(req.params.idObra, " and comentario.idObra=").concat(req.params.idObra);
  connection.query(sql, function (error, results) {
    if (error) {
      debug(error);
      res.sendStatus(500);
